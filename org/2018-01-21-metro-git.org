...
A few years ago, I was trying to learn Clojure, had read some books and was looking
...
So, the idea for this project is to *generate* the necessary git commands
...
The next sections are gonna focus on the journey of achieving this world changing functionality.
So, the post is gonna be broken into the following sections:
1. Build the graph from the input data
2. Traverse the built graph with the required constraints
3. Generate the git commands when traversing the graph
4. Transform our traversal into the Clojure ~seq~ abstraction
...
** Input data format
The data format is gonna be an array of lines and its stations. Simple as that.
...
which provides the foundation of creating and manipulating graphs.
Take a look of some functions we're gonna use in the future.
...
First of all, let's deal with the easiest type of metro map, the one without cycles.

The required code doesn't do much. We're receiving an input data and creating a new loom graph from it
storing the line as attributes of each station/node.
...
** Reversing connections as an alternative
Before start removing connections, we could take a less damaging approach.
As we all know subway lines are bidirectional, which makes sense because
imagine taking the metro to go work and having to take a Uber to come back to your home at the end of the day.

So, after adding a new line and checking if it introduces a cycle,
we could try to reverse the connections.
...
Keep in mind that the call to ~loom.graph/dag?~ does a
...
The idea is to check if there is a cycle for every addition of a origin station and destination station.
If there is, then the destination station is skipped and the station after the destination is connected with the origin station.
...
Notice how the removal of the cycles runs after trying to revert the connections.
...
The algorithm is costly ~O(E*(V+E))~ or ~O(Connections*(Stations+Connection))~ and could be improved,
...
Now we can focus on the traversal of this graph.
...
It's wise to decouple these two components, otherwise the algorithm gets complex really fast.

Now, back to business, these are the requirements to perform this traversal appropriately:
...
** Black box
Instead of having an ~Iterator~ or similar,
the API I thought is a function call that receives
a state of current visited station and lines and a graph and
returns a new state with the visited station and the updated loom graph.
...
Always starting with the simplest case: A subway system with only one line.
...
Not so great. It's just returning the same station every time, but we already have an idea of the parameters and return of the function.

** Single line with predecessors
The next case is to find the unvisited predecessors.
To do so, we recursively try to find a station which doesn't have a unvisited predecessor yet.
...
There is still a problem that the stations of other lines are not even considered when a line visits all stations.
To solve this, we need to introduce a new data structure which holds the stations that are still waiting to be visited, the so-called pending nodes.
...
we add a new key ~end~ to the returned state. In the next iteration, ~nil~ is returned.
...
** Helper function to bootstrap the traversal
One disadvantage of this approach is that the initial state should contain a current-node, including .
To alleviate this, we can encapsulate this logic into its own function.
...
The last piece of the puzzle is to create *one or more git commands* for each new iteration of the traversal.

For each iterated station, the algorithm needs to perform two tasks:
- Switch to or create a branch if needed
- Generate the (merge) commit and, if necessary, move the other branchs to the new commit.

In the switching branch logic:
1. When ~HEAD~ is pointing to some line of the iterated lines, don't do anything.
2. When ~HEAD~ is not pointing to some line but a line already exists, do a ~checkout~ if the line exists or a ~checkout --orphan~ if it doesn't exist.

In the command generation:
1. If there is a single line, create a simple ~commit~.
2. If at least two lines (branches) are coming from different stations (commits), create a ~merge commit~
with the name of the station and move all of the other branches to the new commit.
3. When all the lines (branches) are coming to the same station (commit),
generate a simple ~commit~ and then move the other branches (~branch -f~) to point to this new commit.
...
we need to keep track of where ~HEAD~ is pointing and the commits that the branches are pointing to.

*** Emulating a git repository
Instead of using a real life git repository to keep track of the existing branches and commits,
we can represent it as a plain old Clojure(script) map.
Each new command can update this map with the current state of branches and the commit of each branch.
...
A quick remark is that ~HEAD~ points to a commit in a real git repo,
...
*** Merge branches
When the iterated station has multiple branches and they're coming to different commits, we generate a merge commit.
...
To identify these cases, we can check if the evaluated lines are not from inside the merging branches.
...
One part we call all agree on: the current API to generate these commands really sucks.
...
The clients of this program are only interested in one thing: generate the git commands to a metro configuration.
...
With this new type, we can reuse some ready-made functions that we all learned to love.
...
* That's it, folks
Phew. We reached the end of the journey of mapping git commands of a subway system.
I hope it was a pleasant experience and you learned something new.

I wanna thank the creators of
@@html:<a href="https://github.com/vbarbaresi/MetroGit" target="_blank">MetroGit</a>@@
(Paris) which I got the original idea
and
...
Another great thing about Clojure(script) is that the animations of this post
