...
** Input data format
The data format is gonna be an array of lines and its stations.
...
By the way, some fictitious lines are gonna be represented with colors and stations with letters.
...
which provides an exceptional API to work with graphs.
...
Before start removing some connections, we could take a less damaging approach.
As we all know subway lines are bidirectional, which makes sense because
imagine going to work with a subway and having to take a uber to come back to your home at the end of the day.

So, when a line introduces a cycle, we could try to reverse the connections of the line that introduced a new cycle.
...
which takes ~O(E+V)~.
By the way, ~E~ (edges) are the stations and ~V~ (vertex) are the connections between them.
...
This function only receives commit names and branches and it's totally decoupled from the traversal algorithm.
In the future, we're gonna feed the iterations of the traversal to this command.
...
But, when dealing with multiple branches, 
we need to keep track of where ~HEAD~ is pointing and all of our branches/lines and commits/stations.

*** Emulating a git repository
Instead of using a real life git repository to keep track of the existing branches and commits,
we can represent it as a plain old Clojure map.
...
because, in order to check the need to generate the ~checkout~ commands, 
the algorithm only needs to know the branch that the HEAD is pointing at.
