...
#+CAPTION: Bose QC 35 ii, Pixel 3 and Blue Yeti Nano will be my companions in this adventure. Honourable mention goes to the Bluetooth controller Intel AX200. However, I was too lazy to unplug it from the motherboard to include it in the photo.
...
Again, humans create algorithms in computers to solve a constraint or limitation.
...
Drawing a parallel with TCP/IP protocol, it would be the same as if TCP, IP and Ethernet specifications were all in the same PDF document.
...
To allow both pieces to talk to each other, SIG created the HCI (Host Controller Interface) protocol.
...
In Linux, the translation of ACL/SCO packets and USB packets happens in the [[https://github.com/torvalds/linux/blob/v5.15/drivers/bluetooth/btusb.c][btusb module]].
