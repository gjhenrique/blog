...
#+CAPTION: Bose QC 35 ii, Pixel 3 and Blue Yeti Nano will be my companions in this adventure. Honourable mention goes to the Bluetooth controller Intel AX200. Unfortunately, I was lazy to unplug it from the motherboard.
...
- the source node receives PCM data from the microphone when I speak something
The sink node managing the HFP connection wakes up, receives this PCM data, and encodes the audio with the mSBC codec. Then, it writes this data to a socket. The caller can hear me.
...
- The session manager (~pipewire-media-session~) performs device discovery, policy logic for sandboxed applications, and the node's configuration. It doesn't hold any state of the objects, which is the responsibility of the daemon. The PipeWire session manager is a PoC, and [[https://gitlab.freedesktop.org/pipewire/wireplumber][WirePlumber]] will replace it eventually. This module even lived in a directory called ~examples~ in PipeWire source code, but now it's a separate repo included as a git submodule in the main codebase.
...
Its userspace companion is in a daemon called ~bluetoothd~, which exposes its interface to other apps using some [[https://github.com/bluez/bluez/tree/5.62/doc][D-Bus APIs]].
...
After changing the Java code and fixing some permission logic and SDK version issue, I had a working app.
...
By default, the socket sets the ~sk_sndbuf~ value from ~/proc/sys/net/core/wmem_default~ (value 212992 by default in my machine).
...
I couldn't find the reasoning for setting a low buffer on the PipeWire codebase, but I could trace back why the two-factor is there from a Pulseaudio commit and the [[https://bugs.freedesktop.org/show_bug.cgi?id=58746][forum discussion]].
...
- The number of write syscalls from Bluetooth and its return codes
...
However, it's a bad idea for the callers because of the echo.
